...
const BusRoute = () => {
  ...
  const startChange = (e: React.ChangeEvent<HTMLInputElement>) => {setStart(e.target.value)};

  const goal = useAtomValue(goalAtom);
  const setGoal = useSetAtom(changedGoalValueAtom);
  const goalChange = (e: React.ChangeEvent<HTMLInputElement>) => setGoal(e.target.value);
